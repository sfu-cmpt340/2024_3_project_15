...
# Load datasets
scrolling_datasets = {
    "Scrolling Down": pd.read_csv(
        "../EEG_Data/museFiles_scrolling/original_data/scrolling_down_20_1_jimmy.csv"
    )[["timestamps", "eeg_1", "eeg_2", "eeg_3"]],
    "Scrolling Up": pd.read_csv(
        "../EEG_Data/museFiles_scrolling/original_data/scrolling_up_20_1_jimmy.csv"
    )[["timestamps", "eeg_1", "eeg_2", "eeg_3"]],
}

swiping_datasets = {
    "Swipe Left": pd.read_csv(
        "../EEG_Data/museFiles_swiping/swiping_left_60_1_jimmy.csv"
    )[["timestamps", "eeg_1", "eeg_2", "eeg_3"]],
    "Swipe Right": pd.read_csv(
        "../EEG_Data/museFiles_swiping/swiping_right_60_1_jimmy.csv"
    )[["timestamps", "eeg_1", "eeg_2", "eeg_3"]],
    "Swipe Up": pd.read_csv("../EEG_Data/museFiles_swiping/swiping_up_60_1_jimmy.csv")[
        ["timestamps", "eeg_1", "eeg_2", "eeg_3"]
    ],
    "Swipe Down": pd.read_csv(
        "../EEG_Data/museFiles_swiping/swiping_down_60_1_jimmy.csv"
    )[["timestamps", "eeg_1", "eeg_2", "eeg_3"]],
}

# Initialize dictionary to store extracted features
features = {
    feature: {"scrolling": {}, "swiping": {}}
    for feature in ["mean", "std", "variance", "skewness", "kurtosis", "fourier"]
}
...
for name, dataset in scrolling_datasets.items():
    numeric_data = dataset[["eeg_1", "eeg_2", "eeg_3"]].fillna(
        0
    )  # Ensure only numeric columns and fill NaN values with 0
    pca_data = apply_pca(
        numeric_data[["eeg_1", "eeg_2", "eeg_3"]]
    )  # Apply PCA to EEG1, EEG2, EEG3
    ...
    for dataset_name, feature_values in features[feature_name]["scrolling"].items():
        if len(feature_values) > 0:  # Ensure there is data to plot
            plt.hist(
                ...
                bins=30,
                alpha=0.7,
                label=f"{dataset_name} - {feature_name}",
            )
